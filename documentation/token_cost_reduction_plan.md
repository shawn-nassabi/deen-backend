...
5. **[CORRECTED by the phase-0 live bench]** The original analysis claimed fiqh streaming requests generate the answer twice. Live data disproved this: the streaming fiqh branch in `pipeline_langgraph.py` was **unreachable** — `state_box["final_state"]` kept only the last node's delta (no `fiqh_category`), so fiqh requests fell into the `final_response` short-circuit. Actual production behavior: generation happened exactly once via the non-streaming graph node, **fiqh answers never token-streamed (single blob)** and **the `fiqh_references` SSE event was never emitted**. Fixed in Phase 1 (delta-merge + `streaming_mode` guard) at neutral cost. Fiqh decompose genuinely runs twice per iteration (bench: 24 decompose calls for 9 fiqh queries = 9 graph + 15 retriever) — Phase 4 removes that.
...
1. **Fiqh streaming restoration** *(revised after the phase-0 bench disproved the double-generation claim)*: (a) merge node deltas into `state_box["final_state"]` in the producer loop so the fiqh branch becomes reachable; (b) `if state.get("streaming_mode"): return {}` guard at the top of `_generate_fiqh_response_node` ([chat_agent.py:458-505](agents/core/chat_agent.py#L458-L505)); (c) fix the docstring. Net effect: still exactly one generation per fiqh request (cost-neutral), but fiqh answers now token-stream and the `fiqh_references` SSE event is emitted for the first time in production. Non-streaming `/chat/agentic` unaffected.
