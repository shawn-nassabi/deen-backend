...
_FIQH_BM25_ENCODER_PATH = (
    os.path.join(os.path.dirname(__file__), "data", "fiqh_bm25_encoder.json")
)


def _warn_if_fiqh_encoder_missing() -> None:
    """Surface the silent fiqh-DX failure mode at startup.

    `data/fiqh_bm25_encoder.json` is gitignored and regenerated by the
    Dockerfile (`RUN python scripts/ingest_fiqh.py --encoder-only`). On a
    fresh local clone running `uvicorn main:app --reload` directly, the
    file is absent and fiqh queries silently return the polished refusal
    message because sparse retrieval throws inside
    `modules/fiqh/retriever._get_bm25_encoder`. Logging the remediation
    command at startup turns that into a one-line fix.
    """
    if os.path.exists(_FIQH_BM25_ENCODER_PATH):
        return
    logging.getLogger(__name__).warning(
        "fiqh BM25 encoder missing at %s — fiqh queries will silently fall back "
        "to refusals until you run `python scripts/ingest_fiqh.py --encoder-only`",
        _FIQH_BM25_ENCODER_PATH,
    )
